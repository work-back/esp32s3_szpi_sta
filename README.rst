...
提供 **PyQt 桌面原生工具**（``tools/keymap_editor.py``）与 Web 端（``tools/keymap_editor.html``）两种编辑器。

启动 PyQt 编辑器：

.. code-block:: bash

   python3 tools/keymap_editor.py

特性与配置说明：

* **截图加载与标注**：选择横屏游戏截图或拖拽图片到画布，点击“自适应截图尺寸”设置分辨率。
* **左侧移动摇杆（WASD / 槽位 0）**：
...
编辑完成后点击“导出 map.json”，将导出的文件传给桥接器：
